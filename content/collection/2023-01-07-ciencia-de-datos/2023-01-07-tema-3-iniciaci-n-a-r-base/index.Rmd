...
Los principales tipos son 3: numéricos, de texto y lógicos; en realidad 4 porque R diferencia los datos númericos en doubles e integers^[En realidad son 6 porque R tiene también complex y raw types]
...
La **estructura de datos fundamental en R es el vector**^[De hecho en R no existen los escalares: para R un núnico numero es un vector de 1 elemento]. En R, un vector es una estructura de datos que sirve para almacenar un conjunto ordenado de valores o elementos. Un vector puede contener cualquier número de elementos; sin embargo, **todos los elementos deben ser del mismo tipo**. Por ejemplo, un vector no puede contener números y texto.
...
Como R tiene 4 tipos de datos fundamentales (integer, double, character y lógico)^[En realidad 6 si incluimos complex y raw types] se pueden crear con ellos 4 tipos de vectores; a estos tipos fundamentales de vectores se les suele llamar como "atomic vectors".
...
**PERO** resulta que sobre la base de estos 4 tipos de vectores fundamentales se han construido otros tipos de vectores, que como son derivados de los fundamentales, se les suele llamar (derived or) **augmented vectors**^[Augmented porque se construyen, a partir los vectores fundamentales, añadiendo información extra en los atributos del vector original o fundamental!! Si cuesta un poco entenderlo!! Con tener una ligera idea es suficiente].
...
Con la llegada de R 4.1.0 se produjo un cambio importante en la comunidad R: apareció la **pipe nativa** `>|`^[Como se dice [aquí](https://eliocamp.github.io/codigo-r/en/2021/05/r-pipa-nativa/), la versión 4.0 ya vino con un cambio histórico: se acabo con `stringAsFactors = TRUE`. Además el post sugiere una solución para la falta de placeholder en la pipe nativa]. Una comparación exhaustiva de la pipe nativa con la pipe de magrittr puede encontrarse [aquí](https://michaelbarrowman.co.uk/post/the-new-base-pipe/)
...
Lo anterior mejora un poco (la verdad es que bien poco) si usamos la nueva sintaxis (shorthand) para funciones anónimas o lambda functions^[[Aquí](https://twitter.com/rabaath/status/1335226691304775681) un hilo de Twitter sobre las lambda functions]:
