...
```{r, echo = FALSE}
tweetrmd::tweet_embed("https://twitter.com/achalmaedison/status/1597228787431002113", theme = "light", align = "center", maxwidth = 650)
```
...
Muchas veces en un trabajo se han de calcular estadísticos de **todas las variables** del df. Esto se hace con `summarise()`, PERO utilizando también una función de ayuda (helper function): `across()`^[Hasta la aparición de dplyr 1.0.0 en mayo de 2020, se utilizaba la función `sumarise_all()`]. A veces también hay que usar otra helper function: `where()`.
